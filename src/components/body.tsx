...
const Body: React.FC<{
  filteredData: Movie[];
  children: string;
  sendBookedMovies: any;
}> = ({ filteredData, children, sendBookedMovies }) => {
  return (
    <section className="lg:pr-9 lg:pl-0 md:px-6 md:mt-9 px-4 mt-6 ">
      <h1 className="md:text-[32px] text-[20px] font-light text-white mb-6">
        {children}
      </h1>
      <div className="lg:grid-cols-4 lg:gap-x-10 lg:gap-y-8 md:grid-cols-3 md:gap-x-[29px] md:gap-y-6  grid grid-cols-2 gap-[15px]">
            ...
            <div key={index} className="cursor-pointer">
              <div className="relative rounded-lg caruseldiv overflow-hidden">
                ...
                <div
                  ...
                  className="hover:bg-white svgDiv md:top-4 md:right-4 w-8 h-8 top-2  right-2 flex items-center justify-center bg-bookmarkDiv rounded-[50%] absolute z-30"
                >
                  {!item.isBookmarked ? (
                    <svg
                      width="12"
